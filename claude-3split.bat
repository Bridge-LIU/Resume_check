...
REM 1) Left pane (Claude-1), width 100%%
REM 2) Split vertically (-V) at 0.667 -> new right pane (Claude-2) takes 2/3,
REM    leaving Claude-1 at 1/3
REM 3) Split the focused right pane vertically (-V) at 0.5 -> Claude-3 takes
REM    half of the right 2/3, leaving Claude-2 and Claude-3 each at 1/3
wt -w 0 new-tab -d "%WORKDIR%" --title "Claude-1" cmd /k "set CLAUDE_CODE_NO_FLICKER=1 && claude" ^
  ; split-pane -V -s 0.667 -d "%WORKDIR%" --title "Claude-2" cmd /k "set CLAUDE_CODE_NO_FLICKER=1 && claude" ^
