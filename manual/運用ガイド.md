...
2. **用途に応じて起動 bat をダブルクリック**：
   - **`start-貼付版.bat`** — 貼付モード専用（API キー不要）
   - **`start-完全版.bat`** — 貼付 + API モード両対応（API キー必要）
...
- 終わったら `start-完全版.bat` / `start-貼付版.bat` で起動
...
そのあと `start-完全版.bat` / `start-貼付版.bat` 内の `http://localhost:3939` も合わせて修正。
